        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-6">
            {!user ? (
              ...
              <>
                <button onClick={() => onNavigate('dashboard')} className={`text-sm font-semibold transition-colors hidden md:block ${currentView === 'dashboard' ? 'text-white' : 'text-zinc-400 hover:text-white'}`}>Dashboard</button>
                ...
                <div className="flex items-center gap-3 ml-2 md:ml-4 md:pl-6 md:border-l border-zinc-800">
                  <button 
                    ...
                    className={`w-10 h-10 rounded-full flex items-center justify-center border transition-colors ${currentView === 'profile' ? 'bg-rose-500/20 border-rose-500 text-rose-500' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:text-white'}`}
                  >
